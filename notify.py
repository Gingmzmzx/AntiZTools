...
class NotificationWindow(QListWidget):
    ...
    @classmethod
    def _createInstance(cls, app):
        # 创建实例
        if not cls._instance:
            cls._instance = NotificationWindow(app=app)
            cls._instance.show()
            NotificationIcon.init()

    @classmethod
    def info(cls, app, title, message, callback=None):
        cls._createInstance(app)
    ...
    @classmethod
    def success(cls, app, title, message, callback=None):
        cls._createInstance(app)
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, cls._instance,
    ...
    @classmethod
    def warning(cls, app, title, message, callback=None):
        cls._createInstance(app)
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, cls._instance,
    ...
    @classmethod
    def error(cls, app, title, message, callback=None):
        cls._createInstance(app)
        item = QListWidgetItem(cls._instance)
...
class WindowNotify(QWidget, Ui_NotifyForm):
    ...
    def _init(self, app):
        ...
        self.animation.setDuration(500)  # 1s
...
if __name__ == '__main__':
    ...
    layout.addWidget(QPushButton(
        'Info', w, clicked=lambda: NotificationWindow.info(app, '提示', '这是一条会自动关闭的消息', callback=callback)))
    layout.addWidget(QPushButton(
        'Success', w, clicked=lambda: NotificationWindow.success(app, '提示', '这是一条会自动关闭的消息', callback=callback)))
    layout.addWidget(QPushButton(
